...
void part1(int numPoints)
{
    ...
    for(Point p:genPoints) {
        outfile << setprecision(23) << p.getX() << "  " << p.getY() << endl;
    }
    outfile.close();
...
bool compare (Point p1,Point p2) { return (p1.getX()<p2.getX());}
...
double minFromStrip(vector<Point> &points, Point &p1, Point &p2, double middle, double minDistanceP, int lowA, int highA) {
    ...
    i=0;
    while(high == -1 && middleL+i<=highA) {
        if(highX < points[middleL+i].getX()) {
            high = middleL+i-1;
        }
        i++;
    }
...
//     } else {
//         return minDistanceStrip(points, p1, p2, low,middle-2, high);
// //     }
}
...
double recursiveMinDistanceFind(vector<Point> &points, Point &p1, Point &p2, int low, int high) {
    if(high-low > 3) {
        ...
        if(distance1 < distance2) {
            double distance3 = minFromStrip(points, *p3, *p4, middle, distance1, low, high);
            ...
            double distance3 = minFromStrip(points, *p3, *p4, middle, distance2, low, high);
...
void logData(int minNodes, int maxNodes, int avgCase, int multiplier) {
    ...
    for(int i =minNodes; i<maxNodes; i=i*multiplier) {
        ...
        outfile << i <<","<<avgTime1/3<<","<<avgTime2/3<<"\n";
        avgTime1 = 0;
        avgTime2 = 0;
    }
    outfile.close();
